...
1. Search for `'lessfoodie'` (inside single quotations) to capture the text domain.
2. Search for `lessfoodie_` to capture all the function names.
3. Search for <code>&nbsp;_tk</code> (with a space before it) to capture DocBlocks.
4. Search for `lessfoodie-` to capture prefixed handles.

OR

* Search for: `'lessfoodie'` and replace with: `'megatherium'`
* Search for: `lessfoodie_` and replace with: `megatherium_`
